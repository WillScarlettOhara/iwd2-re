    ...
    if (nBytesCopied == 0 && nTotalBytes == 0) {
        ...
        if (bEnabled == TRUE) {
            for (int side = 0; side < 4; side++) {
                ...
                m_mosBorders[side].m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;
            }

            if (bTravel) {
                m_mosBackground.SetResRef(CResRef("GTRSCRN"), FALSE, TRUE);
                m_mosBackground.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;
                ...
                if (g_pBaldurChitin->GetObjectGame()->GetRuleTables().m_tAreaLoad.Find(g_pBaldurChitin->GetObjectGame()->field_1B86, ptLocation, TRUE)) {
                    ...
                    if (sBackground != "*") {
                        ...
                        m_mosTravelArea.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;
                    }
                }
            } else {
            ...
            m_mosProgressBarSkull.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosProgressBarSkullHighlighted.SetResRef(CResRef("GTRBPSK2"), FALSE, TRUE);
            m_mosProgressBarSkullHighlighted.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosProgressBarBackground.SetResRef(CResRef("GTRBPBG"), FALSE, TRUE);
            m_mosProgressBarBackground.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosProgressBarForeground.SetResRef(CResRef("GTRBPBAR"), FALSE, TRUE);
            m_mosProgressBarForeground.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosProgressBarCap.SetResRef(CResRef("GTRBPCAP"), FALSE, TRUE);
            m_mosProgressBarCap.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_vidFont.SetResRef(CResRef("NORMAL"), g_pBaldurChitin->m_bUseNewGui, FALSE);
            m_vidFont.RegisterFont();
            m_vidFont.SetColor(RGB(255, 255, 255), RGB(0, 0, 0), FALSE);

            m_initialsFont.SetResRef(CResRef("INITIALS"), g_pBaldurChitin->m_bUseNewGui, FALSE);
            m_initialsFont.RegisterFont();

            m_parchmentFont.SetResRef(CResRef("INFOFONT"), g_pBaldurChitin->m_bUseNewGui, FALSE);
            m_parchmentFont.RegisterFont();
            m_parchmentFont.SetColor(RGB(255, 255, 255), RGB(10, 10, 10), FALSE);

            m_skullAnimating.SetResRef(CResRef("SKULANIM"), g_pBaldurChitin->m_bUseNewGui, FALSE, TRUE);

            m_mosMinibarSkull.SetResRef(CResRef("GTRSPSK"), FALSE, TRUE);
            m_mosMinibarSkull.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosMinibarSkullHighlighted.SetResRef(CResRef("GTRSPSK2"), FALSE, TRUE);
            m_mosMinibarSkullHighlighted.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosMinibarBackground.SetResRef(CResRef("GTRSPBG"), FALSE, TRUE);
            m_mosMinibarBackground.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosMinibarForeground.SetResRef(CResRef("GTRSPBAR"), FALSE, TRUE);
            m_mosMinibarForeground.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

            m_mosMinibarForegroundCap.SetResRef(CResRef("GTRSPCAP"), FALSE, TRUE);
            m_mosMinibarForegroundCap.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;
