    ...
    : CUIControlBase(panel, &(controlInfo->base), 1)
{
    ...
    m_nField142 = static_cast<short>(max(wm_field_140 - thumbSize.cy, 1));
    m_nRenderCount = 0;
...
void CUIControlScrollBar::AdjustScrollBar(SHORT a1, SHORT a2, SHORT a3)
{
    if (!bm_field_146) {
        if (a2 - a3 > 0) {
            wm_field_144 = min(m_nField142 * a1 / (a2 - a3), m_nField142);
            InvalidateRect();
        } else {
            if (a2 != 0) {
                wm_field_144 = m_nField142 * a1 / a2;
                InvalidateRect();
            } else {
                wm_field_144 = 0;
                InvalidateRect();
...
void CUIControlScrollBar::OnLButtonUp(CPoint pt)
{
    if (bm_field_146) {
        if (m_nTextDisplayID != -1) {
            ...
            if (pText->m_plstStrings->GetCount() <= pText->m_wFieldA6C) {
                if (wm_field_144 != 0) {
                    wm_field_144 = 0;
                    InvalidateRect();
                }
...
void CUIControlScrollBar::OnMouseMove(CPoint pt)
{
    if (m_bActive && bm_field_146) {
        ...
        if (v1 >= v2) {
            if (v1 <= v2 + m_nField142) {
                wm_field_144 = v1 - v2;
                OnScroll();
            } else {
                wm_field_144 = m_nField142;
                OnScroll();
            }
        } else {
            wm_field_144 = 0;
...
void CUIControlScrollBar::OnScroll()
{
    if (m_nTextDisplayID != -1) {
        CUIControlTextDisplay* pText = static_cast<CUIControlTextDisplay*>(m_pPanel->GetControl(m_nTextDisplayID));
        pText->OnScroll(wm_field_144, m_nField142);
    }
}
...
void CUIControlEditScrollBar::OnScroll()
{
    if (m_nTextDisplayID != -1) {
        ...
        // NOTE: Uninline.
        pEdit->OnScroll(wm_field_144, m_nField142);
    }
}
