    ...
    while (pos != NULL) {
        ...
        if (rc == CGameObjectArray::SUCCESS) {
            if (pObject->GetObjectType() != CGameObject::TYPE_SPRITE
                || static_cast<CGameSprite*>(pObject)->GetBaseStats()->bm_field_294 != 1) {
...
void CGameArea::Unmarshal(BYTE* pArea, LONG areaSize, const CString& sName, BOOLEAN bProgressBarInPlace)
{
    ...
    // VARIABLES
    CVariable variable;
    g_pBaldurChitin->GetObjectGame()->bm_field_1B84 = 1;
    ...
    g_pBaldurChitin->GetObjectGame()->bm_field_1B84 = 0;
    ...
    g_pBaldurChitin->GetObjectGame()->sm_field_1B86 = "";
}
...
void CGameArea::OnActivation()
{
    ...
    if (g_pBaldurChitin->GetObjectGame()->GetVisibleArea() == this) {
        ...
        if (g_pBaldurChitin->bm_field_1C4A == 1 && byte_8D2138 == TRUE) {
        ...
        if (m_nCurrentSong != -1) {
            if (!g_pBaldurChitin->cSoundMixer.GetSoundChannelState()) {
...
void CGameArea::OnDeactivation()
{
    ...
    if (!g_pBaldurChitin->bm_field_1C4A) {
...
void CGameArea::OnActionButtonDown(const CPoint& pt)
{
    ...
    if (pt.x >= m_cInfinity.rViewPort.left
        ...
        if (m_pGame->GetState() == 0) {
            if (m_pGame->GetButtonArray()->nm_field_19B2 != 0) {
