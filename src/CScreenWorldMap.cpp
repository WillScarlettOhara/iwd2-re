...
void CScreenWorldMap::SetMapView(const CPoint& ptMapView)
{
    ...
    if (pGame->IsSpecialArea(sArea)) {
        ...
        CSize cMapSize = pWorldMap->GetMapSize(pWorldMap->GetCurrentMapIndex());
...
void CScreenWorldMap::OnMapMouseDown(const CPoint& ptMousePos)
{
    ...
    if (m_nEngineState == 1
        ...
        if (m_nSelectedArea != -1) {
            CWorldMapArea* pArea = pWorldMap->GetArea(pWorldMap->GetCurrentMapIndex(), m_nSelectedArea);
...
BOOL CScreenWorldMap::DrawMap(const CRect& r)
{
    ...
    if (m_aAreaRect.GetCount() != 0) {
        ...
        CSize mapSize = pWorldMap->GetMapSize(pWorldMap->GetCurrentMapIndex());
        ...
        if (pVidInf->BKLock(rClip)) {
            ...
            for (DWORD nArea = 0; nArea < pWorldMap->GetNumAreas(pWorldMap->GetCurrentMapIndex()); nArea++) {
                CWorldMapArea* pArea = pWorldMap->GetArea(pWorldMap->GetCurrentMapIndex(), nArea);
                    ...
                    if (!rAreaDirty.IsRectEmpty()) {
                        ...
                        if (!m_bMapDragging
                            ...
                            if (nArea == m_nSelectedArea && m_bOverSelectedArea) {
                                ...
                                rgbColor = AdjustPulseColor(rgbColor);
                            }
                        }
...
void CScreenWorldMap::StartWorldMap(INT nEngineState, LONG nLeavingEdge, BOOLEAN bInControl)
{
    ...
    m_vmMap.SetResRef(CResRef(pWorldMap->GetMap(pWorldMap->GetCurrentMapIndex())->m_resMosaic), FALSE, TRUE);
    m_vmMap.m_bDoubleSize = g_pBaldurChitin->m_bUseNewGui;

    // NOTE: Uninline.
    m_vcAreas.SetResRef(CResRef(pWorldMap->GetMap(pWorldMap->GetCurrentMapIndex())->m_resAreaIcons),
    ...
    // NOTE: Uninline.
    m_aAreaRect.SetSize(pWorldMap->GetNumAreas(pWorldMap->GetCurrentMapIndex()));

    for (DWORD nArea = 0; nArea < pWorldMap->GetNumAreas(pWorldMap->GetCurrentMapIndex()); nArea++) {
        m_aAreaRect[nArea] = GetAreaRect(pWorldMap->GetCurrentMapIndex(), nArea);
    }
...
// 0x69CB40
COLORREF CScreenWorldMap::AdjustPulseColor(COLORREF rgb)
{
...
DWORD CScreenWorldMap::FindAreaHit(const CPoint& pt)
{
    ...
    DWORD nNumAreas = pWorldMap->GetNumAreas(pWorldMap->GetCurrentMapIndex());
    for (DWORD nArea = 0; nArea < nNumAreas; nArea++) {
        CRect rArea = m_aAreaRect[nArea];
        if (rArea.PtInRect(ptMapPos)
            && (pWorldMap->GetArea(pWorldMap->GetCurrentMapIndex(), nArea)->m_dwFlags & 0x4) != 0) {
            nFoundArea = nArea;
            break;
        }
    }
...
void CScreenWorldMap::GetMarkerPosition(CPoint& ptMarker)
{
    ...
    if (m_nCurrentLink != -1) {
        DWORD nSrcArea = pWorldMap->FindSourceAreaIndex(pWorldMap->GetCurrentMapIndex(), m_nCurrentLink);
        DWORD nDstArea = pWorldMap->GetLink(pWorldMap->GetCurrentMapIndex(), m_nCurrentLink)->m_nArea;
        CWorldMapArea* pSrcArea = pWorldMap->GetArea(pWorldMap->GetCurrentMapIndex(), nSrcArea);
        CWorldMapArea* pDstArea = pWorldMap->GetArea(pWorldMap->GetCurrentMapIndex(), nDstArea);
        ...
        DWORD nArea;
        if (pWorldMap->GetAreaIndex(pWorldMap->GetCurrentMapIndex(), m_cResCurrentArea, nArea)
            || pWorldMap->GetAreaIndex(pWorldMap->GetCurrentMapIndex(), CResRef("AR2626"), nArea)) {
            CRect rArea = m_aAreaRect[nArea];
