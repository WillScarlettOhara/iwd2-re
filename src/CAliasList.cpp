...
BOOL CAliasList::AddAlias(CAlias* pAlias)
{
    ...
    if (pAlias->nm_field_4) {
        AddTail(pAlias);
    }

    return pAlias->nm_field_4;
}
...
BOOL CAliasList::ResolveFileName(const CString& a2, CString& a3)
{
    ...
    while (aliasPos != NULL) {
        ...
        if (pAlias != NULL) {
            if (a2.Find(pAlias->nm_field_8) != -1) {
                POSITION pos = pAlias->nfield_C.GetHeadPosition();
                if (pos != NULL) {
                    while (pos != NULL) {
                        temp = pAlias->nfield_C.GetNext(pos);
                        ...
                        a3 = a2.Right(a2.GetLength() - pAlias->nm_field_8.GetLength());
                        a3 = temp + a3;
