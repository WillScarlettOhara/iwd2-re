...
BOOL CAliasList::ResolveFileName(const CString& a2, CString& a3)
{
    ...
    while (aliasPos != NULL) {
        ...
        if (pAlias != NULL) {
            if (a2.Find(pAlias->m_sAlias) != -1) {
                POSITION pos = pAlias->m_lPaths.GetHeadPosition();
                if (pos != NULL) {
                    while (pos != NULL) {
                        temp = pAlias->m_lPaths.GetNext(pos);
                        ...
                        a3 = a2.Right(a2.GetLength() - pAlias->m_sAlias.GetLength());
                        a3 = temp + a3;
